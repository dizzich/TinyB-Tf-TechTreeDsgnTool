...
/** Notion logo icon. Document shape and black parts (N + bookmark) can be themed via props. */
export function NotionIcon({
  ...
  className?: string;
  size?: number;
  /** Replaces only the black parts (N and bookmark). */
  color?: string;
  /** Override document shape fill (e.g. match background so only N/bookmark show). */
  docFill?: string;
}) {
  const accentFill = color ?? '#000000';
  const documentFill = docFill ?? 'var(--notion-doc-fill)';
  return (
    <svg
      width={size}
      height={size}
      ...
      aria-hidden
    >
      {/* Document shape: use docFill so it can match background (e.g. in StartupModal). */}
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        ...
        style={{ fill: documentFill }}
      />
      {/* N: originally #000000 — only this is replaced by `color` */}
      <path
        ...
        fill={accentFill}
      />
      {/* Bookmark shape: originally #000000 — only this is replaced by `color` */}
      <path
        fillRule="evenodd"
        clipRule="evenodd"
